...
void drawGlideSlopeBall(float position){
  ...
  if( (destination.glideSlope <= -1.200 )&&(destination.glideSlope >= -2.0) ){
      // grab coordinates of leftest
      glVertex3f(0.135, -0.04, 0.0); // #1
      glVertex3f(0.125, -0.04, 0.0); // #2
      glVertex3f(0.125, -0.03, 0.0); // #3
      glVertex3f(0.135, -0.03, 0.0);
  }
  else if((destination.glideSlope <= -0.4)&&(destination.glideSlope >-1.2)){
      // grab second to left most
      glVertex3f(0.135, 0.0, 0.0); // #1
      glVertex3f(0.125, 0.0, 0.0); // #2
      glVertex3f(0.125, 0.01, 0.0); // #3
      glVertex3f(0.135, 0.01, 0.0);
  }
  else if((destination.glideSlope <= 1.2)&&(destination.glideSlope >= 0.4)){
    // grab second to right
    glVertex3f(0.135, 0.08, 0.0); // #1
    glVertex3f(0.125, 0.08, 0.0); // #2
    glVertex3f(0.125, 0.07, 0.0); // #3
    glVertex3f(0.135, 0.07, 0.0);
  }
  else if((destination.glideSlope <= 2)&&(destination.glideSlope > 1.2)){
    // grab rightmost
    //glVertex3f ( x , y , z)
    glVertex3f(0.135, 0.12, 0.0); // #1
    glVertex3f(0.125, 0.12, 0.0); // #2
    glVertex3f(0.125, 0.11, 0.0); // #3
    glVertex3f(0.135, 0.11, 0.0);
  }
  else{
    // put in center
    glVertex3f(0.135, 0.045, 0.0); // #1
    glVertex3f(0.125, 0.045, 0.0); // #2
    glVertex3f(0.125, 0.035, 0.0); // #3
    glVertex3f(0.135, 0.035, 0.0);
  }
...
void drawLocalizerBox(float position){
        ...
        glBegin(GL_POLYGON);
        glColor3f(127.0, 0.0, 255.0);
        if( (destination.localizerScale <= -1.200 )&&(destination.localizerScale >= -2.0) ){
            // grab coordinates of leftest
            glVertex3f(-0.078, -0.14, 0.0); // #1
            glVertex3f(-0.07, -0.14, 0.0); // #2
            glVertex3f(-0.07, -0.13, 0.0); // #3
            glVertex3f(-0.078, -0.13, 0.0);
        }
        else if((destination.localizerScale <= -0.4)&&(destination.localizerScale >-1.2)){
            // grab second to left most
        ...
        else if((destination.localizerScale <= 1.2)&&(destination.localizerScale >= 0.4)){
          // grab second to right
        ...
        else if((destination.localizerScale <= 2)&&(destination.localizerScale > 1.2)){
          // grab rightmost
          //glVertex3f ( x , y , z)
...
void drawSlipBall(float position){
        ...
        // This controls translation: 1st arg +right, 2nd arg +up, 3rd arg +closer
        // This controls translation: 1st arg +right, 2nd arg +up, 3rd arg +closer
        //if( destination.slipSkid <= 1.50 && destination.slipSkid >= -1.50){
          glTranslatef((( destination.slipSkid / 10) ),  1.60, 2.0);
        //} // Setup texture binding between the references.
        // Create the sphere with the texture and longitude & lat. divisions,
        gluSphere(planeBall, .05, 10, 10);
        // Pop x2
